...
#    bar = Bar('   Processing', max=len(page_soup.findAll("div",{"class":"categoryArticle__content"})))
    for categoryArticle__content in tqdm(page_soup.findAll("div",{"class":"categoryArticle__content"})):
        ...
        if this_date == latest_date:
            ...
            if os.path.exists(root_dir+"/oil_price_news_data/"+"cnbc_news_"+d+".csv"):
                output_df.to_csv(root_dir+"/oil_price_news_data/"+"cnbc_news_"+d+".csv",mode='a',header=False)
            else:
                output_df.to_csv(root_dir+"/oil_price_news_data/"+"cnbc_news_"+d+".csv")
